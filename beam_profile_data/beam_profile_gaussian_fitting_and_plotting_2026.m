...
function FWHM = plot_data_and_fit(table_data, model, background)
    ...
    plot(x, intensity-background, Color='red', Marker='o', ...
        LineStyle='none', MarkerFaceColor='red', MarkerSize=4)
    hold on
    plot(my_fit, 'k')
    legend('Data', 'Gaussian fit')
    ylabel('Pixel greyscale brightness')
    xlabel('Distance (mm)')
end
...
function fig = fit_gaussian_and_plot( ...
    ...
    fig = figure(Position=[100 100 600 700]);
        subplot(2, 1, 1);
            ...
            title(sprintf('Thin axis, FWHM = %.2f mm', FWHM_thin))
        subplot(2, 1, 2);
            FWHM_long = plot_data_and_fit(table_long, model_long, background_long);
            title(sprintf('Long axis,  FWHM = %.2f mm', FWHM_long))
