...
def compute_tfm_separate_masks(worker_id, fmc_3d, tfm_params, time_vector_us, signal_progress):
    ...
    x_grid_m, z_grid_m = build_tfm_grid(
        tfm_params.grid_size_x_mm, tfm_params.grid_size_z_mm, tfm_params.n_pixels_z)
    ...
    # If requested, create ray angle masks:
    if tfm_params.mask_tf:
        # Some kind of mask has been requested.
        # Get the relevant MaskBehaviours for each of gen and det:
        # Gen:
        if tfm_params.mask_behaviour_string_gen:
            mask_behaviour_gen = dict_mask_behaviours[tfm_params.mask_behaviour_string_gen]
            numpy_masking_function_gen = mask_behaviour_gen.numpy_masking_function
            angles_rad_masked_gen = create_tfm_masks(x_grid_m, z_grid_m, x_elements_m,
                                                     tfm_params.mask_angle_deg,
                                                     numpy_masking_function_gen)
        else:
            angles_rad_masked_gen = None
        # Det:
        if tfm_params.mask_behaviour_string_det:
            mask_behaviour_det = dict_mask_behaviours[tfm_params.mask_behaviour_string_det]
            numpy_masking_function_det = mask_behaviour_det.numpy_masking_function
            angles_rad_masked_det = create_tfm_masks(x_grid_m, z_grid_m, x_elements_m,
                                                     tfm_params.mask_angle_deg,
                                                     numpy_masking_function_det)
        else:
            angles_rad_masked_det = None
    else:
    ...
    displacement_image_complex = np.zeros(np.shape(x_grid_m))
    ...
    for det_index in range(n_tx):
        ...
        for gen_index in range(n_tx):
            ...
            # gen_index:
            delays_for_this_a_scan_s = (times_send_s[gen_index] +
                                        times_receive_s[det_index])
            ...
            if tfm_params.mask_tf:
                # Get gen angle mask:
                if angles_rad_masked_gen is None:
                    mask_gen = np.ma.nomask
                else:
                    mask_gen = np.ma.getmask(angles_rad_masked_gen[gen_index])
                # Get det angle mask:
                if angles_rad_masked_det is None:
                    mask_det = np.ma.nomask
                else:
                    mask_det = np.ma.getmask(angles_rad_masked_det[det_index])
                # Combine gen and det masks:
                mask_gen_and_det = np.ma.mask_or(mask_gen, mask_det)
                # Apply combined mask to sampled amps:
                sampled_amps_complex_processed = np.ma.masked_where(mask_gen_and_det,
                                                                    sampled_amps_complex,
                                                                    copy=False)
            else:
            ...
            displacement_image_complex = displacement_image_complex + np.ma.filled(sampled_amps_complex_processed,
                                                                             fill_value=0)

            if gen_index == 25 and det_index == 50:
                plt.imshow(np.real(sampled_amps_complex_processed))
                # plt.imshow(np.real(sampled_amps_filled))
                plt.show()
                a=1
    ...
    # Return the image of summed displacements back to the script calling this function.
    return displacement_image_complex, fmc_3d_filtered
