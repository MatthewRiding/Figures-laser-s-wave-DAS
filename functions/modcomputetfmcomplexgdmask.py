...
def compute_tfm_complex_gd_masks_same(worker_id, fmc_3d, tfm_params, time_vector_us, signal_progress):
    ...
    if numpy_masking_function:
        angles_rad_masked = create_tfm_masks(x_grid_m, z_grid_m, x_elements_m, tfm_params.mask_angle_deg,
                                             numpy_masking_function)
    else:
        angles_rad_masked = None
    ...
    for det_index in range(n_tx):
        ...
        for gen_index in range(n_tx):
            ...
            if angles_rad_masked is not None:
                mask_gen = np.ma.getmask(angles_rad_masked[gen_index])
                mask_det = np.ma.getmask(angles_rad_masked[det_index])
                ...
                if flip_polarity_where_masked:
                    ...
                    polarity_flip_factor = np.where(mask_gen_and_det, -1, 1)
